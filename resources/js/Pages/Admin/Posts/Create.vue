        ...
        <div class="py-12">
            <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div class="mb-8">
                    ...
                    <p class="text-gray-600 mt-2">Create a post and publish it to multiple platforms</p>
                </div>
                    ...
                    <div class="lg:col-span-1">
                        ...
                        <div class="bg-white rounded-lg shadow-sm p-6 mb-6">
                            <div class="flex justify-between items-center mb-4">
                                ...
                                <Link href="/platforms" class="text-xs bg-blue-50 text-blue-700 hover:bg-blue-100 px-3 py-1 rounded transition">
                                    ⚙️ Manage
                                </Link>
                            </div>
                            
                            <div v-if="platforms.length > 0" class="space-y-3">
                                <div v-for="platform in platforms" :key="platform.id" class="flex items-center p-2 rounded hover:bg-gray-50 transition">
                                    <input
                                        :id="`platform-${platform.id}`"
                                        type="checkbox"
                                        :value="platform.id"
                                        v-model.number="form.platforms"
                                        class="w-4 h-4 text-blue-600 rounded focus:ring-2 focus:ring-blue-500"
                                    />
                                    <label :for="`platform-${platform.id}`" class="ml-3 text-sm font-medium text-gray-700 cursor-pointer capitalize flex-1">
                                        {{ platform.name }}
                                    </label>
                                    <span class="text-xs text-gray-500">{{ platform.slug }}</span>
                                </div>
                            </div>

                            <div v-else class="p-4 bg-yellow-50 border border-yellow-200 rounded text-sm text-yellow-800">
                                ⚠️ No active platforms available. <Link href="/platforms/create" class="underline font-semibold">Create one</Link>
                            </div>
